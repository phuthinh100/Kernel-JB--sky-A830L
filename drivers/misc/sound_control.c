...
int boost = 0;
int boost_limit = 3;
int boost_limit_min = 0;

int headset_boost = 0;
int headset_boost_limit = 8;
int headset_boost_limit_min = 0;
...
static ssize_t headset_boost_store(struct device * dev, struct device_attribute * attr, const char * buf, size_t size)
{
	...
	if (new_val != headset_boost) {
		...
		update_headset_volume_boost(headset_boost_limit - headset_boost);
	}
