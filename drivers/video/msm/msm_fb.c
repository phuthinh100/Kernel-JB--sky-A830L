...
#ifdef CONFIG_F_SKYDISP_DONOT_FREE_PIPE_SUB_BLANK_FORCE
...
#if defined(CONFIG_F_SKYDISP_VEIL_VIEW) 
extern void mipi_rohm_veil_mode(boolean enable);
extern int mipi_rohm_veil_set(struct msmfb_veil_view veil);
#endif
...
#elif defined(CONFIG_MACH_MSM8960_STARQ)|| defined(CONFIG_MACH_MSM8960_EF45K) || \
		defined(CONFIG_MACH_MSM8960_EF46L) || defined(CONFIG_MACH_MSM8960_EF47S)
	struct fb_info *fbi = mfd->fbi;
			...
			unsigned long arg)
{
...
#endif
	int ret = 0;
#if defined(CONFIG_F_SKYDISP_LCD_FORCE_ONOFF) ||defined(CONFIG_F_SKYDISP_VEIL_VIEW) 
	boolean	enable;
#if defined(CONFIG_F_SKYDISP_VEIL_VIEW) 
	...
	case MSMFB_SKY_VEIL_ENABLE:
		ret = copy_from_user(&enable, argp, sizeof(enable));
		printk(KERN_INFO "[LIVED] veil enable/disable enable=%d\n", enable);
		down(&msm_fb_ioctl_ppp_sem);
		mipi_rohm_veil_mode(enable);
#ifdef CONFIG_F_SKYDISP_DONOT_FREE_PIPE_SUB_BLANK_FORCE
		dont_free_pipe_while_sub_blank_force=1;
#endif				
		msm_fb_blank_sub_force(0, info, 1);
	...
	case MSMFB_SKY_VEIL_SET:
		...
		printk(KERN_INFO "[LIVED] veil set\n");
		down(&msm_fb_ioctl_ppp_sem);
		ret = mipi_rohm_veil_set(veil);
		if (ret == 1) {
#ifdef CONFIG_F_SKYDISP_DONOT_FREE_PIPE_SUB_BLANK_FORCE
		dont_free_pipe_while_sub_blank_force=1;
#endif					
